...
- [x] **B1 · `deltaSince(rev)` — HUB DONE.** The hub owns a monotonic `rev` (bumps per changing push), stamps each changed record `_rev`, and serves `GET /api/db?since=N` = only what moved (an idempotent re-push doesn't bump the rev or re-broadcast). `rev` on the health poll + POST reply. `tests/delta-sync.js`. ↳ *engine pending:* pull with `?since` and merge the delta in (client-side `mergeArr`) instead of adopting the whole DB.
